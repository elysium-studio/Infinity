...
public sealed class DesktopModule : IModule
{
    ...
    private static DesktopScrollPreviewView CreateDesktopScrollPreviewView(IServiceProvider provider)
    {
        ...
        return new(windowPreviewSurface, windowCollection, panState, pager, scroller, workspace, scrollInputSuppression, backgroundSource, overviewConfiguration, foregroundThemeResolver, animator, chromeAnimator, clockController, layoutPresenter, pageStrip, previews, cursorConfinement, shortcutHints, applicationPicker, applicationDock, applicationDockContextMenuBuilder, applicationDockPressAnimator, applicationLaunchCoordinator, inputController, snapInteractionCoordinator, provider.GetRequiredService<DesktopOverlayBoundaryResizeController>(), provider.GetRequiredService<DesktopWindowDragFrames>(), provider.GetRequiredService<DesktopWindowContentSearchController>(), logger);
    }
    ...
    private static DesktopOverviewView CreateDesktopOverviewView(IServiceProvider provider)
    {
        ...
        DesktopOverviewConfiguration overviewConfiguration = provider.GetRequiredService<DesktopOverviewConfiguration>();
        return new(desktopScrollPreview, backdropAnimator, wallpaperPresenter, inputController, backgroundSource, keyboardInputSource, windowEventListener, overviewConfiguration, provider.GetRequiredService<IModifierKeyState>(), provider.GetRequiredService<IPointerInputSource>(), provider.GetRequiredService<IWindowDragGuard>(), provider.GetRequiredService<IWindowStore>(), provider.GetRequiredService<IWindowGeometryReader>(), provider.GetRequiredService<ITrackedWindowDragController>(), provider.GetRequiredService<ILogger<DesktopContentDragController>>(), provider.GetRequiredService<ILogger<DesktopLiveWindowDragController>>());
    }
