...
{
    int? maxPages = options.VirtualPagesMode == VirtualPagesMode.Fixed ? (int? )options.VirtualPagesCount : null;
    provider.GetRequiredService<IPager>().SetMaxPages(maxPages);
    provider.GetRequiredService<IPanState>().SetMaxOffset(maxPages.HasValue ? (maxPages.Value - 1) * (double)DisplayArea.Primary.WorkArea.Width : double.MaxValue);
}).WithChangeHandler((provider, options, _) => provider.GetRequiredService<DesktopOverviewDragScrollerConfiguration>().SpeedLevel = options.DragScrollSpeed).WithChangeHandler((provider, options, _) =>
{
...
{
    IConfigurationBuilder configBuilder = new ConfigurationBuilder().SetBasePath(provider.GetRequiredService<IHostEnvironment>().ContentRootPath).AddJsonFile("settings.dat", optional: true, reloadOnChange: true);
