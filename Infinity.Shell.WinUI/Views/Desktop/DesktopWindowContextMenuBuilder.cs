...
public sealed class DesktopWindowContextMenuBuilder(IWindowCollection windowCollection, IPager pager, IWorkspace workspace, PageTitleStore pageTitleStore, PageLayoutStore pageLayoutStore, DesktopPageStrip pageStrip, DesktopSnapLayoutCatalog layoutCatalog, DesktopWindowPlacementCoordinator placementCoordinator, ITextLocalizer localizer)
{
    ...
    private void Populate(MenuFlyout flyout, nint windowHandle)
    {
        ...
        flyout.Items.Add(CreateItem(localizer.GetText("DesktopWindowMinimize"), () => placementCoordinator.TryMinimize(windowHandle), commandState.CanMinimize));
        flyout.Items.Add(commandState.CanRestore ? CreateItem(localizer.GetText("DesktopWindowRestore"), () => placementCoordinator.TryRestore(windowHandle)) : CreateItem(localizer.GetText("DesktopWindowMaximize"), () => placementCoordinator.TryMaximize(windowHandle), commandState.CanMaximize));
        flyout.Items.Add(new MenuFlyoutSeparator());
        flyout.Items.Add(CreateItem(localizer.GetText("DesktopWindowBringToCurrentPage"), () => placementCoordinator.TryMoveToPage(windowHandle, currentPage, center: false), windowPage != currentPage));
        MenuFlyoutSubItem sendToPage = new()
        {
            Text = localizer.GetText("DesktopWindowSendToPage")
        };
        ...
        {
            int targetPage = page;
            sendToPage.Items.Add(CreateItem(pageTitleStore.GetTitle(page), () => placementCoordinator.TryMoveToPage(windowHandle, targetPage, center: false), page != windowPage));
        }
        ...
        if (!pager.MaxPages.HasValue)
        {
            ...
            sendToPage.Items.Add(CreateItem(localizer.GetText("DesktopWindowNewPage"), () => placementCoordinator.TryMoveToPage(windowHandle, newPage, center: false)));
        }
        ...
        if (definition is not null)
        {
            ...
            {
                int targetSlot = slot;
                moveToSlot.Items.Add(CreateItem(localizer.GetText("DesktopWindowSlot", slot + 1), () => placementCoordinator.TryMoveToSlot(windowHandle, windowPage, layout, targetSlot, workspace.WorkAreaX, workspace.WorkAreaY)));
            }
        }

        flyout.Items.Add(moveToSlot);
        TrackedWindow[] swapTargets = [..windowCollection.AllTrackedWindows.Where(candidate => candidate.Handle != windowHandle && placementCoordinator.GetPage(candidate) == windowPage).OrderBy(candidate => candidate.Title).ThenBy(candidate => (long)candidate.Handle)];
