...
public sealed class DesktopWallpaperColorSampler
{
    ...
    {
        if (string.IsNullOrWhiteSpace(wallpaperPath) || monitorWidth <= 0 || monitorHeight <= 0 || !double.IsFinite(monitorPoint.X) || !double.IsFinite(monitorPoint.Y))
        {
            return null;
        }
        ...
        (string Path, long Length, long Modified) key = (file.FullName.ToUpperInvariant(), file.Length, file.LastWriteTimeUtc.Ticks);
        ...
        catch
        {
            lock (cacheGate)
            {
                if (samples.TryGetValue(key, out Task<DesktopWallpaperSample?>? current) && ReferenceEquals(current, pending))
                {
                    samples.Remove(key);
                }
            }
        ...
        uint? color = sample?.Sample(monitorWidth, monitorHeight, monitorPoint.X, monitorPoint.Y, DesktopWallpaperBrushFactory.WindowsFillVerticalAlignment);
        return color is uint argb ? Color.FromArgb((byte)(argb >> 24), (byte)(argb >> 16), (byte)(argb >> 8), (byte)argb) : null;
    }
    ...
    private static async Task<DesktopWallpaperSample?> DecodeAsync(string path)
    {
        ...
        PixelDataProvider provider = await decoder.GetPixelDataAsync(BitmapPixelFormat.Bgra8, BitmapAlphaMode.Straight, transform, ExifOrientationMode.RespectExifOrientation, ColorManagementMode.ColorManageToSRgb);
        bool rotated = decoder.OrientedPixelWidth != decoder.PixelWidth;
        return new((int)(rotated ? height : width), (int)(rotated ? width : height), provider.DetachPixelData());
    }
}
