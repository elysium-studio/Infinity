...
// Animate the XAML hand-in properties, never the hand-out element Visual:
// XAML continues to own CenterPoint, Translation and Scale on these hosts.
internal sealed class DesktopWindowPlacementAnimator(params UIElement[] elements) : IDisposable
{
    ...
    {
        if (batch is null) return fallback;

        // Retarget a second arrange from the in-flight bounds rather than
        // jumping to the previous destination. Matches the compositor easing.
        ...
        for (int iteration = 0; iteration < 16; iteration++)
        {
            ...
            if (curveX < time) low = t; else high = t;
        }
        double parameter = (low + high) / 2;
        double progress = parameter * parameter * (3 - 2 * parameter);
        return new(source.X + (target.X - source.X) * progress,
    ...
    {
        Stop();
        if (!from.IsValid || !to.IsValid || from == to || !settings.AnimationsEnabled) return;
        ...
        // Scale is around the final XAML CenterPoint. Compensate translation
        // so both the old top-left and old size are preserved at frame zero.
        Vector3 start = new((float)(from.X - to.Width / 2 * (1 - scale.X)),
            (float)(from.Y - to.Height / 2 * (1 - scale.Y)), depth);
        ...
        {
            if (ReferenceEquals(batch, current)) Stop();
        };
    ...
    public void Stop()
    {
        if (batch is null && translationAnimation is null && scaleAnimation is null) return;
        CompositionScopedBatch? previous = batch;
        batch = null;
        foreach (UIElement element in elements)
        {
            if (translationAnimation is not null) element.StopAnimation(translationAnimation);
            if (scaleAnimation is not null) element.StopAnimation(scaleAnimation);
        }
        previous?.Dispose();
