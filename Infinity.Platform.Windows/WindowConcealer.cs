...
public sealed unsafe class WindowConcealer(IWindowEnumerator enumerator, ILogger<WindowConcealer> logger) : IWindowConcealer, IWindowConcealmentRecovery, IDisposable
{
    ...
    public void RecoverStrandedWindows() => enumerator.EnumerateVisible(windowHandle =>  {  HWND hwnd = new(windowHandle);  lock (syncRoot)  {  if (isDisposed || concealedWindows.ContainsKey(windowHandle))  {  return;  }   if (!TryReadRecoveryOrigin(hwnd, out WindowOrigin origin))  {  return;  }   if (!Move(windowHandle, origin.X, origin.Y))  {  logger.LogWarning("Could not recover a concealed window. Handle={WindowHandle}, Error={Error}", windowHandle, Marshal.GetLastPInvokeError());  return;  }   ClearRecoveryOrigin(hwnd);  logger.LogInformation("Recovered a window concealed by a previous session. Handle={WindowHandle}, X={WindowX}, Y={WindowY}", windowHandle, origin.X, origin.Y);  }  });
    ...
    public bool Conceal(nint windowHandle)
    {
        ...
        lock (syncRoot)
        {
            ...
            if (!Move(windowHandle, OffscreenParkX, OffscreenParkY))
            {
                ...
                logger.LogWarning("Could not conceal a window. Handle={WindowHandle}, Error={Error}", windowHandle, Marshal.GetLastPInvokeError());
                return false;
            }
    ...
    public void Dispose()
    {
        ...
        foreach ((nint windowHandle, WindowOrigin origin)in strandedWindows)
        {
            ...
            {
                logger.LogWarning("Could not restore a concealed window during shutdown. Handle={WindowHandle}, Error={Error}", windowHandle, Marshal.GetLastPInvokeError());
            }
        }
    ...
    {
        if (!SetWindowProperty(hwnd, RecoveryXProperty, new HANDLE(origin.X)) || !SetWindowProperty(hwnd, RecoveryYProperty, new HANDLE(origin.Y)) || !SetWindowProperty(hwnd, RecoveryMarkerProperty, new HANDLE(1)))
        {
            int error = Marshal.GetLastPInvokeError();
            ClearRecoveryOrigin(hwnd);
            logger.LogWarning("Could not record concealed-window recovery data. Handle={WindowHandle}, Error={Error}", (nint)hwnd, error);
            return false;
        }
    ...
    private static bool TryGetWindowOrigin(HWND hwnd, out WindowOrigin origin)
    {
        ...
        origin = new(rectangle.left, rectangle.top);
        return true;
    }
