...
public sealed class WindowCloser(ILogger<WindowCloser> logger) : IWindowCloser, IDisposable
{
    ...
    private const SET_WINDOW_POS_FLAGS VisibilityFlags =
        SET_WINDOW_POS_FLAGS.SWP_NOMOVE | SET_WINDOW_POS_FLAGS.SWP_NOSIZE |
        SET_WINDOW_POS_FLAGS.SWP_NOZORDER | SET_WINDOW_POS_FLAGS.SWP_NOOWNERZORDER |
        SET_WINDOW_POS_FLAGS.SWP_NOACTIVATE | SET_WINDOW_POS_FLAGS.SWP_ASYNCWINDOWPOS;
    ...
    public bool TryClose(nint windowHandle)
    {
        ...
        if (windowHandle == 0 || !PInvoke.IsWindow(hwnd)) return false;
        ...
        lock (gate)
        {
            ...
            if (pending.ContainsKey(windowHandle)) return true;

            int token = unchecked(++nextToken);
            if (token == 0) token = unchecked(++nextToken);
    ...
    private async Task CloseFrameAsync(HWND hwnd, int token)
    {
        try
        {
            // Hiding happens BEFORE SC_CLOSE: reasserting the overlay's HWND order
            // cannot suppress a separate shell closing representation. NOACTIVATE
            // is essential here; ShowWindow(SW_HIDE) can activate another window.
            lock (gate)
            {
                if (disposed || !OwnsMarker(hwnd, token)) return;
            ...
            if (!OwnsMarker(hwnd, token)) return;
            ...
            // Wait for the hidden frame to reach DWM, not just its window styles.
            // Neither a foreign HWND nor DWM may block Infinity's UI thread.
            HRESULT flushResult = await Task.Run(() => PInvoke.DwmFlush())
    ...
    private static bool SetVisibility(HWND hwnd, bool visible) => PInvoke.SetWindowPos(hwnd, default, 0, 0, 0, 0,
        VisibilityFlags | (visible ? SET_WINDOW_POS_FLAGS.SWP_SHOWWINDOW : SET_WINDOW_POS_FLAGS.SWP_HIDEWINDOW));
    ...
    {
        fixed (char* name = CloseMarker) return PInvoke.SetProp(hwnd, name, new HANDLE(token));
    }

    private static unsafe bool OwnsMarker(HWND hwnd, int token)
    {
        fixed (char* name = CloseMarker) return PInvoke.GetProp(hwnd, name) == new HANDLE(token);
    }

    private unsafe void RestoreFrame(HWND hwnd, int token)
    {
        if (!OwnsMarker(hwnd, token)) return; // Destroyed/reused HWNDs must not be shown.

        // A cancelled close, confirmation prompt, or unresponsive app must not
        // remain hidden. Queue show even if hide hasn't been processed yet.
        if (!SetVisibility(hwnd, true))
        {
            logger.LogError("Could not restore packaged window after close. Handle={WindowHandle}, Error={Error}",
                (nint)hwnd, Marshal.GetLastPInvokeError());
            _ = PInvoke.ShowWindowAsync(hwnd, SHOW_WINDOW_CMD.SW_SHOWNOACTIVATE);
        }
        fixed (char* name = CloseMarker) PInvoke.RemoveProp(hwnd, name);
    }
    ...
    public void Dispose()
    {
        ...
        {
            disposed = true;
            foreach ((nint hwnd, int token) in pending) RestoreFrame(new HWND(hwnd), token);
            pending.Clear();
        }
    }
}
