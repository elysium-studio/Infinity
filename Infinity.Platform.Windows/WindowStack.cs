...
public sealed class WindowStack(IWindowStore repository, IWindowEventListener listener, Func<nint> handleFactory, IDispatcher dispatcher, ILogger<WindowStack> logger) : IWindowStack
{
    ...
    private readonly Dictionary<nint, int> windowStackMap = [];
    ...
    private void HandleWindowStackChanged() => ScheduleRefresh();
    ...
    private bool TrySetWindowPos(HWND hwnd, HWND insertAfter, SET_WINDOW_POS_FLAGS flags, string operation)
    {
        try
        {
            ...
            logger.LogDebug("SetWindowPos failed while trying to {Operation}. Hwnd: {Hwnd}, InsertAfter: {InsertAfter}, Error: {Error}", operation, (nint)hwnd, (nint)insertAfter, error);
            return false;
        }
        catch (Exception exception)
        {
            logger.LogDebug(exception, "SetWindowPos threw while trying to {Operation}. Hwnd: {Hwnd}, InsertAfter: {InsertAfter}", operation, (nint)hwnd, (nint)insertAfter);
            return false;
        }
    }
