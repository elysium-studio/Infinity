...
public static class IServiceCollectionExtensions
{
    extension(IServiceCollection services)
    {
        public IServiceCollection AddInfinityApplication()
        {
            ...
            services.AddSingleton<IScroller>(provider =>  {  IScrollTimer scrollTimer = provider.GetRequiredService<IScrollTimer>();  return new Scroller(provider.GetRequiredService<IPanState>(), provider.GetRequiredService<IScrollPresentationSession>(), provider.GetRequiredService<IWindowStore>(), provider.GetRequiredService<IWindowMover>(), provider.GetRequiredService<IWindowMoveGuard>(), provider.GetRequiredService<IWindowDragGuard>(), provider.GetRequiredService<IScrollInputSource>(), provider.GetRequiredService<IDispatcher>(), provider.GetRequiredService<Func<ScrollerConfiguration>>(), new EasingScrollMotion(), new FluentNavigationScrollMotion(TimeProvider.System), new MomentumScrollMotion(), provider.GetRequiredService<IPageCenterTargetResolver>(), scrollTimer.Start, scrollTimer.Stop, provider.GetRequiredService<ILogger<Scroller>>());  });
