...
public sealed class DesktopSnapSlotOccupancyResolverTests
{
    ...
    public void GroupSelectionIsExcludedWithoutCopyingOrChangingTheSet()
    {
        ...
        Assert.True(resolver.TryGetOccupant(placement, selection.SelectedHandles,
            [first, second, third], out TrackedWindow? result));
        Assert.Same(third, result);
        Assert.Equal(2, selection.SelectedHandles.Count);

        selection.ToggleSelected(3);
        Assert.False(resolver.TryGetOccupant(placement, selection.SelectedHandles,
            [first, second, third], out result));
        Assert.Null(result);
    }
