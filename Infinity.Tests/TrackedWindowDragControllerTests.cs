...
public sealed class TrackedWindowDragControllerTests
{
    ...
    public void EndClearsOnlyMatchingDrag()
    {
        ...
        TrackedWindow secondWindow = CreateWindow(500, 200)with
        {
            Handle = new(2)
        };
        store.Add(window);
        store.Add(secondWindow);
        TrackedWindowDragController controller = new(store, new TestScroller(), new TestWindowResizeSynchronizer(), NullLogger<TrackedWindowDragController>.Instance);
