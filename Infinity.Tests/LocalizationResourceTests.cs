...
{
    private static readonly string[] ExpectedLanguages = ["de", "en", "es", "fr", "it", "ja", "ko", "nl", "pl", "pt-BR", "qps-ploc", "zh-Hans", ];
    ...
    public void LocalizedResourcesMatchEnglishContracts()
    {
        ...
        string[] languages = [..Directory.GetDirectories(root).Select(Path.GetFileName).OfType<string>().Order()];
    ...
    private static IReadOnlyList<ResourceEntry> ReadResources(string path) => XDocument.Load(path).Root!.Elements("data").Select(element => new ResourceEntry(element.Attribute("name")!.Value, element.Element("value")!.Value)).ToArray();
