...
public class WindowPageCoordinatorTests
{
    ...
    public void OverviewSelectionAlignsTheContainingPageInsteadOfTheWindowCentre()
    {
        ...
        WindowPageCoordinator coordinator = CreateCoordinator(store, scroller, activator);
        IntPtr handle = new(5);
        store.Add(new TrackedWindow { Handle = handle, CanvasX = 1750, CanvasY = 0, Width = 500, Height = 500 });
        coordinator.NavigateTo(handle);
    ...
    private static WindowPageCoordinator CreateCoordinator(IWindowStore store, IScroller scroller, IWindowActivator activator) => new(store, scroller, new TestWorkspace(), activator, new TestDispatcher(), new WindowPageGeometry());

    private static TrackedWindow CreateWindow(IntPtr handle, int canvasX) => new()
    {
        Handle = handle,
        CanvasX = canvasX,
        CanvasY = 0,
