...
public sealed class DesktopApplicationDockViewModelTests
{
    ...
    public async Task SaveOrderPersistsTheListViewCollectionOrder()
    {
        ...
        DesktopApplicationDockViewModel viewModel = new(new TestPinStore([first, second, third, fourth]), orderStore, new PendingDockCatalog(), new TestApplicationCatalog(), new TestDispatcher());
        viewModel.Applications.Move(0, 2);
        viewModel.Applications.Move(3, 1);
        await viewModel.SaveOrderAsync();
        Assert.Equal([second.Id, fourth.Id, third.Id, first.Id], viewModel.Applications.Select(item => item.Application.Id));
        Assert.Equal([second.Id, fourth.Id, third.Id, first.Id], orderStore.ApplicationIdentifiers);
    }
    ...
    private sealed class TestOrderStore : IDesktopApplicationDockOrderStore
    {
        ...
        public Task SaveAsync(IEnumerable<string> applicationIdentifiers, CancellationToken cancellationToken = default)
        {
    ...
    {
        public Task<IReadOnlyList<DesktopApplicationDockEntry>> GetApplicationsAsync(int maximumCount, CancellationToken cancellationToken = default) => new TaskCompletionSource<IReadOnlyList<DesktopApplicationDockEntry>>().Task;
    }


    private sealed class TestApplicationCatalog : IDesktopApplicationPickerCatalog
    {
        public Task<IReadOnlyList<LaunchableApplication>> GetApplicationsAsync(CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyList<LaunchableApplication>>([]);
