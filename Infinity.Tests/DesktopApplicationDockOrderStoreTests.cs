...
public sealed class DesktopApplicationDockOrderStoreTests
{
    ...
    public async Task SavedOrderIsNormalizedAndPersisted()
    {
        ...
        DesktopApplicationDockOrderStore store = new(
            new TestOptionsMonitor(settings),
            new TestWritableOptions(settings),
            NullLogger<DesktopApplicationDockOrderStore>.Instance);
    ...
    {
        Settings settings = new() { DockApplicationOrder = ["terminal", "edge"] };
        DesktopApplicationDockOrderStore store = new(
            new TestOptionsMonitor(settings),
            new TestWritableOptions(settings),
            NullLogger<DesktopApplicationDockOrderStore>.Instance);
