...
public sealed class DesktopBackgroundSourceTests
{
    ...
    private static DesktopBackgroundSource CreateSource(TestLogger logger, Func<DesktopBackgroundSnapshot> snapshotReader) => new(logger, snapshotReader, PollingInterval, RecoveryPollingInterval, false);

    private static DesktopBackgroundSnapshot CreateSnapshot(uint colour) => new(string.Empty, colour);
