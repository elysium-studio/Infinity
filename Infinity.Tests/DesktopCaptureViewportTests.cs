...
{
    private static readonly DesktopCaptureViewport Viewport =
        DesktopCaptureViewport.Create(1920, 1080, 1920, 1040, 0, 0, 0.38);

    [Theory]
    [InlineData(-1200)]
    [InlineData(0)]
    [InlineData(3000)]
    public void IncludesVisibleAndPartiallyVisibleThumbnails(double x)
    {
        Assert.True(Viewport.ShouldCapture(x, 100, 500, 400, false));
    }
    ...
    public void AccountsForTaskbarOffsetsAndTheWholeZoomPath(
        double offsetX, double offsetY, double workWidth, double workHeight)
    {
        DesktopCaptureViewport viewport = DesktopCaptureViewport.Create(
            1920, 1080, workWidth, workHeight, offsetX, offsetY, 0.38);
        foreach (double scale in new[] { 0.38, 0.6, 1.0 })
        {
