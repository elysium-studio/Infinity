...
internal sealed class DesktopOverlayMonitorTopology
{
    ...
    public DesktopOverlayMonitorSpan ResolveSpan(HMONITOR activeMonitor, bool spanningEnabled)
    {
        ...
        if (monitorIndex < 0)
        {
            return new DesktopOverlayMonitorSpan(default, new HashSet<nint>());
        }
        ...
        List<DesktopOverlayMonitor> compatible = [.. monitors
            .Where(monitor => IsCompatible(activeDisplay, monitor))
    ...
    private static DesktopOverlayMonitorSpan CreateSpan(IReadOnlyList<DesktopOverlayMonitor> monitors)
    {
        ...
        return new DesktopOverlayMonitorSpan(bounds, handles);
    }

    private static bool IsCompatible(DesktopOverlayMonitor active, DesktopOverlayMonitor candidate) =>
        candidate.Bounds.Width == active.Bounds.Width &&
        candidate.Bounds.Height == active.Bounds.Height &&
        candidate.Bounds.Y == active.Bounds.Y &&
        candidate.DpiX == active.DpiX &&
        candidate.DpiY == active.DpiY;
