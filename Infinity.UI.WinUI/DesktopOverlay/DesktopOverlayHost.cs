...
internal class DesktopOverlayHost
{
    ...
    internal MonitorHandle CurrentMonitor => new((nint)currentMonitor);
    ...
    public void SetStaysOpen(bool value) => staysOpen = value;
    ...
    private static Window CreateOverlayWindow()
    {
        ...
        _ = PInvoke.SetWindowLong(handle, WINDOW_LONG_PTR_INDEX.GWL_STYLE, style & ~0x00C00000 & ~0x00040000 & ~0x00080000);
        WindowStyle exStyle = (WindowStyle)PInvoke.GetWindowLong(handle, WINDOW_LONG_PTR_INDEX.GWL_EXSTYLE);
        _ = PInvoke.SetWindowLong(handle, WINDOW_LONG_PTR_INDEX.GWL_EXSTYLE, (int)(exStyle | WindowStyle.Transparent | WindowStyle.NoActivate | WindowStyle.ToolWindow | WindowStyle.Layered));
    ...
    private static void PrewarmWindow(Window window, RectInt32 rect, SystemBackdropElement backdropElement, Grid contentRoot)
    {
        HWND handle = new(WindowNative.GetWindowHandle(window));
    ...
    private void ApplyInputState(Window window, HMONITOR monitor)
    {
        ...
        WindowStyle updated = isInputEnabled && monitor == currentMonitor ? style & ~WindowStyle.Transparent : style | WindowStyle.Transparent;
        if (updated != style)
        {
    ...
    private void AnimateWindowOpacity(DispatcherQueue dispatcherQueue, HWND handle, float from, float to, TimeSpan duration, Action completed)
    {
