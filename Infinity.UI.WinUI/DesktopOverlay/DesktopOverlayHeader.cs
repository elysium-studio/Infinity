...
internal class DesktopOverlayHeader
{
    ...
    public DesktopOverlayHeader()
    {
        ...
        handle = new(WindowNative.GetWindowHandle(window));
        WindowStyle exStyle = (WindowStyle)PInvoke.GetWindowLong(handle, WINDOW_LONG_PTR_INDEX.GWL_EXSTYLE);
        PInvoke.SetWindowLong(handle, WINDOW_LONG_PTR_INDEX.GWL_EXSTYLE, (int)(exStyle | WindowStyle.Layered | WindowStyle.NoActivate | WindowStyle.ToolWindow));
    ...
    private static float EaseInOut(float progress) => progress < 0.5f ? 2f * progress * progress : 1f - MathF.Pow(-2f * progress + 2f, 2f) / 2f;
    ...
    private static RectInt32 GetMonitorWorkArea(HMONITOR monitor)
    {
        ...
        return new(info.rcWork.left, info.rcWork.top, info.rcWork.right - info.rcWork.left, info.rcWork.bottom - info.rcWork.top);
    }
    ...
    private void RepositionToWorkArea()
    {
        ...
        int y = placement == DesktopOverlayHeaderPlacement.Bottom ? workArea.Y + workArea.Height - windowHeight : workArea.Y;
        PInvoke.SetWindowPos(handle, HwndTopmost, x, y, windowWidth, windowHeight, SwpNoActivate);
        WindowExtensions.SetBorderless(handle, true);
    }
