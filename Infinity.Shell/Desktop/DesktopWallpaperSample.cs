...
public sealed record DesktopWallpaperSample(int Width, int Height, byte[] Pixels)
{
    ...
    {
        if (Width <= 0 || Height <= 0 || Pixels.LongLength != (long)Width * Height * 4 || monitorWidth <= 0 || monitorHeight <= 0 || !double.IsFinite(x) || !double.IsFinite(y) || !double.IsFinite(verticalAlignment))
        {
            return null;
        }
        ...
        return alpha == 0 ? null : 0xff000000u | ((uint)(red / alpha) << 16) | ((uint)(green / alpha) << 8) | (uint)(blue / alpha);
    }
}
