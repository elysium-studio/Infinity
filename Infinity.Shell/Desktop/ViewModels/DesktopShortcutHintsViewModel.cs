...
public sealed partial class DesktopShortcutHintsViewModel :
    ObservableObject,
    IRecipient<OptionsChangedEventArgs<Settings>>
{
    ...
    public void Receive(OptionsChangedEventArgs<Settings> message) =>
        ApplyModifiers(message.Options.ScrollModifierKeys);

    private void ApplyModifiers(List<List<int>>? combinations)
    {
        List<int> keys = combinations?
            .Where(combination => combination.Count > 0)
