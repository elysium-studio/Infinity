...
public sealed partial class DesktopApplicationDockViewModel : ObservableObject
{
    ...
    public bool CanPin(LaunchableApplication application)
    {
        ...
        return !Applications.Any(item => string.Equals(item.Application.Id, application.Id, StringComparison.OrdinalIgnoreCase));
    }
    ...
    {
        ArgumentNullException.ThrowIfNull(application);
        DesktopApplicationDockItemViewModel? existing = Applications.FirstOrDefault(item => string.Equals(item.Application.Id, application.Id, StringComparison.OrdinalIgnoreCase));
    ...
    public async Task UnpinAsync(DesktopApplicationDockItemViewModel item, CancellationToken cancellationToken = default)
    {
    ...
    public Task SaveOrderAsync(CancellationToken cancellationToken = default) => orderStore.SaveAsync(Applications.Select(item => item.Application.Id), cancellationToken);

    public async Task LoadIconAsync(DesktopApplicationDockItemViewModel item, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(item);
    ...
    {
        IReadOnlyList<DesktopApplicationDockEntry> applications = await dockCatalog.GetApplicationsAsync(MaximumApplications);
