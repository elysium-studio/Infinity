...
public sealed class DesktopApplicationDockOrderStore : IDesktopApplicationDockOrderStore
{
    ...
    public async Task SaveAsync(IEnumerable<string> identifiers, CancellationToken cancellationToken = default)
    {
    ...
    private static List<string> Normalize(IEnumerable<string> identifiers) => [..identifiers.Where(identifier => !string.IsNullOrWhiteSpace(identifier)).Distinct(StringComparer.OrdinalIgnoreCase).Take(MaximumApplications)];
}
